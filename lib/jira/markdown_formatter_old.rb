...
module Jira
  ...
  class MarkdownFormatter
    ...
    # Fallback method that uses the V1 formatter if API conversion fails
    ...
    def self.convert_with_fallback(markdown_text, jira_base_url: nil, auth_token: nil, auth_email: nil)
      begin
        ...
        # Load the V1 formatter for fallback
        require_relative 'markdown_formatter/v1'
        Jira::MarkdownFormatter::V1.convert_markdown_to_adf(markdown_text)
      end
    end
    ...
    def self.setup_instructions
      <<~INSTRUCTIONS
        To use Jira::MarkdownFormatter, set these environment variables:
        ...
        Jira::MarkdownFormatter.convert_markdown_to_adf(
          markdown_text,
