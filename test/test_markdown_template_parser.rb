...
class TestMarkdownTemplateParser < Minitest::Test
  ...
  def test_parses_basic_template_structure
    ...
    assert result.key?(:project_key)
    assert result.key?(:priority)
    assert result.key?(:labels)
    assert result.key?(:description_section)
    assert result.key?(:references_section)
    assert result.key?(:acceptance_criteria_section)
  end
  ...
  def test_extracts_project_information
    ...
    assert_equal %w[test automation], result[:labels]
  end
  ...
  def test_parses_different_project_keys
    ...
    assert_equal %w[urgent bug-fix], result[:labels]
  end
  ...
  def test_extracts_metadata_with_various_formats
    ...
    assert_equal %w[cleanup technical-debt], result[:labels] # Splits and trims
    assert result.key?(:parent_issue)
